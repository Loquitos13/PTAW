...
document.addEventListener("DOMContentLoaded", function () {
    ...
    async function getRecentOrders() {
        ...
        const orderColors = {
            Pending:    { textColor: 'rgb(204, 154, 0)', backgroundColor: 'rgba(255, 193, 7, 0.5)' },
            Received:   { textColor: 'rgb(13, 71, 161)', backgroundColor: 'rgba(33, 150, 243, 0.5)' },
            Confirmed:  { textColor: 'rgb(27, 94, 32)', backgroundColor: 'rgba(76, 175, 80, 0.5)' },
            Completed:  { textColor: 'rgb(66, 66, 66)', backgroundColor: 'rgba(158, 158, 158, 0.5)' },
            Sent:       { textColor: 'rgb(0, 96, 128)', backgroundColor: 'rgba(91, 192, 222, 0.5)' }, 
            Pendente:   { textColor: 'rgb(204, 154, 0)', backgroundColor: 'rgba(255, 193, 7, 0.5)' },
            Recebido:   { textColor: 'rgb(13, 71, 161)', backgroundColor: 'rgba(33, 150, 243, 0.5)' },
            Confirmado: { textColor: 'rgb(27, 94, 32)', backgroundColor: 'rgba(76, 175, 80, 0.5)' },
            Completo:   { textColor: 'rgb(66, 66, 66)', backgroundColor: 'rgba(158, 158, 158, 0.5)' },
            Enviado:    { textColor: 'rgb(0, 96, 128)', backgroundColor: 'rgba(91, 192, 222, 0.5)' }
        };
            ...
            if (table) {
                ...
                result.forEach(item => {
                    ...
                    const colorObj = orderColors[item.Status];
                    ...
                    if (colorObj) {
                        ...
                        statusTd = `<td style="
                            ...
                            background-color: ${colorObj.backgroundColor};
                            font-weight: bold;
                        ">${item.Status}</td>`;

                    } else {

                        statusTd = `<td>${item.Status}</td>`;

                    }

                    tr.innerHTML = `
                        <td scope="row"><b>${item.OrderID}</b></td>
                        <td>${item.Customer}</td>
                        ${statusTd}
            ...
            const parent = document.getElementById("recent_orders_div");
            if (parent) {

                const placeholder = document.createElement("div");
                placeholder.className = "placeholder-graph";
                placeholder.textContent = "No Recent Orders Found";
    ...
    async function recentOrders() {
        ...
        const response = await fetch('../../admin/getRecentOrders.php', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
