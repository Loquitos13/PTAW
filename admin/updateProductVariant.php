...
function updateDimensionData($data) {
    ...
    if (empty($data) || !is_array($data) || empty($data['id_produto']) || empty($data['id_cor'])) {
        throw new Exception("Os campos 'id_produto' e 'id_cor' são obrigatórios para atualização da variante de produto");
    }

    $payload = json_encode($data);

    $ch = curl_init("$apiUrl/updateProductVariants");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
