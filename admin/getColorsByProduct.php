...
try {
    $response = getCoresByProductData();
...
function getCoresByProductData() {
    global $apiUrl;
    ...
    $ch = curl_init("$apiUrl/getColorsByProduct/$idProduto");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    if (!$decodedResponse || !$decodedResponse['success']) {
        throw new Exception($decodedResponse['message'] ?? "Erro ao obter cores do produto");
    }

    return [
        'cores' => $decodedResponse['cores'] ?? []
    ];
}
?>